...
std::string convert_wstr_to_str(std::wstring wstr) // wstringŸ���� utf-8 string���� ��ȯ�մϴ�.
{
...
std::vector<Course> CourseDatabase::query(CourseQuery condition) const
{
    ...
    for (const auto course_ptr : matched_courses)
    {
        ...
        if (is_matched && !condition.professors.empty()) // ���� ���ǿ� �´��� Ȯ��
        {
            is_matched = condition.professors.find(course_ptr->get_professor()) !=
                         condition.professors.end(); // O(logn) Ž���ð� �ҿ�
        }
