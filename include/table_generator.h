...
class TableGenerator
{
  private:
    ...
    unordered_set<string> existCourse; // 생성한 시간표에 들어간 강의이름

    int currentGrade = 0; // 지금까지 넣은 강의의 총 학점수
    int totalGrade; // 설정한 총 학점수
    vector<CourseTime> time; // 생성한 시간표에 채워진 시간

    bool findTime(Course course); // 겹치는 시간있는지 확인
    bool findCourse(Course course); // 겹치는 강의있는지 확인
  public:
    void setTotalGrade(int i); // 총 학점수 설정

    void setQuery(CourseQuery query); // 쿼리 설정 공강, 학년, 학기등을 설정합니다.
